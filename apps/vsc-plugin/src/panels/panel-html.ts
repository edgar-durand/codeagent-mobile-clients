...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Auto-show pairing privacy: when the IDE auto-generates a code
       (panel opened, not yet paired) the QR + 6-digit code are
       rendered blurred so an idle panel can't leak a scannable code
       during pair programming / screen shares. User clicks the
       "Click to reveal" pill to unblur. Reveal sticks until the
       panel becomes hidden again (next refresh will re-blur). */
    .pairing-secret {
      position: relative;
    }
    .pairing-secret.blurred .pairing-secret-content {
      filter: blur(10px);
      user-select: none;
      pointer-events: none;
    }
    .pairing-secret-reveal {
      position: absolute;
      inset: 0;
      display: none;
      align-items: center;
    ...
    .pairing-secret-reveal-pill {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      padding: 8px 14px;
      border-radius: 999px;
      background: var(--ca-purple-soft);
      border: 1px solid var(--ca-glow-purple);
      box-shadow: 0 0 18px var(--ca-purple-glow-soft);
      backdrop-filter: blur(2px);
    }
    .pairing-secret.blurred .pairing-secret-reveal {
      display: flex;
    }
    .pairing-secret-reveal:focus-visible .pairing-secret-reveal-pill {
      outline: 2px solid var(--ca-purple);
      outline-offset: 2px;
    }
    .session-row {
...
<body>
  <div id="disconnected-view" role="region" aria-label="Pairing">
    ...
    <div id="pairing-section" class="card hidden" aria-live="polite">
      <h3>Pairing Code</h3>
      <div id="pairing-secret" class="pairing-secret">
        <div class="pairing-secret-content">
        ...
        <button
          id="pairing-reveal-btn"
          type="button"
          class="pairing-secret-reveal"
          aria-label="Reveal pairing code"
        >
          <span class="pairing-secret-reveal-pill">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
            Click to reveal
          </span>
        </button>
      </div>
  ...
  <script nonce="${nonce}">
    ...
    window.addEventListener('message', (event) => {
      ...
      switch (msg.type) {
        ...
        case 'pairingCode': {
          ...
          // Auto-generated codes ship blurred — the user clicks
          // "Click to reveal" to expose the QR / code, so an idle
          // panel can't leak a scannable secret in a screen share.
          // Manual codes (user clicked Generate / Regenerate) ship
          // unblurred — the click was an explicit "I want to pair
          // right now" signal.
          const secret = document.getElementById('pairing-secret');
          if (msg.autoGenerated) {
            secret.classList.add('blurred');
          } else {
            secret.classList.remove('blurred');
          }
          // Stash the latest timer so an earlier refresh doesn't keep
          // ticking against a stale element after a code refresh.
          if (window.__pairingTimerHandle) {
            clearInterval(window.__pairingTimerHandle);
          }
          const timer = document.getElementById('pairing-timer');
          const expiresAt = msg.expiresAt;
          window.__pairingTimerHandle = setInterval(() => {
            const remaining = Math.max(0, Math.floor((expiresAt - Date.now()) / 1000));
            if (remaining <= 0) {
              clearInterval(window.__pairingTimerHandle);
              window.__pairingTimerHandle = null;
    ...
    // Reveal the auto-blurred pairing code on explicit user action.
    // Stays revealed until the next refresh (which re-applies blur
    // if still auto-generated) or until the panel is reopened.
