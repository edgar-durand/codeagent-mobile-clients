 ...
 * (`sessionId`, `pluginId`, `filePath`, `fileStatus`) tacked on by
 * the caller.
 *
 * Ported verbatim from `apps/cli/src/services/file-watcher/diff-parser.ts`
 * — keep the two in sync. The VS Code plugin keeps its own copy so the
 * file-watcher service is self-contained and the shared package doesn't
 * grow a Node-specific helper.
 */
