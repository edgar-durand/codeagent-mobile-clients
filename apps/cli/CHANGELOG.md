...
- **cli:** Prewarm installs yarn projects' deps at session start too (#826)

## [2.75.30] — 2026-09-25

### Fixed

- **cli:** Preview error log tail without terminal escape codes (#823)

## [2.75.29] — 2026-09-25

### Fixed

- **cli:** Preview reaches the real dev server (readiness, Host), no CodeAgent Cloud copy (#821)

## [2.75.28] — 2026-09-25

### Fixed

- **cli:** Preview proxy forwards to the address the dev server really serves (#820)
...
- **shared:** Preview origin + preview_agent_highlight wire types
- **cli:** Agent-driven Preview — codeagent_preview MCP tools over a loopback bridge
...
- **cli:** Preview detection shows the agent's own error instead of 'returned nothing'

## [2.75.22] — 2026-09-25

### Fixed

- **cli:** House agents read the Agent Standard from their own config dir

## [2.75.21] — 2026-09-25

### Fixed

- **cli:** Close ACP tool rows whose completion carries no content

### Tests

- **cli:** Build opencode config paths with path.join so Windows CI passes

## [2.75.20] — 2026-09-25

### Fixed

- **cli:** Preview heals a committed host-allow shim instead of wrapping it

## [2.75.19] — 2026-09-25

### Fixed

- **cli:** Authenticate gh on self-hosted boxes with a token lacking read:org

## [2.75.18] — 2026-09-25

### Fixed

- **cli:** Skip bd bootstrap on an empty remote; agent must not repair beads
- **cli:** Beads mint must not write agent files or git hooks into the repo

## [2.75.17] — 2026-09-25

### Fixed

- **cli:** Host-agent respawns a session child that crashed after it was up

## [2.75.16] — 2026-09-25

### Fixed

- **cli:** One-shots (Preview detect, summaries) run as the CURRENT agent, not the deploy one
...
- **cli:** Bound streaming-chunk snapshots under the API cap and coalesce big re-sends

## [2.75.10] — 2026-09-23

### Fixed

- **cli:** Guardrails judge a write by its target; pack stage shows a mid-turn permission as awaiting
...
- **release:** Un reintento del publicado de VS Code tiene que poder avanzar (#771)

### Chore

- **cli:** Cerrar los restos de la retirada de Headroom (#773)
- **cli:** Borrar el provisionamiento de Python, muerto desde la retirada de Headroom (#774)

## [2.75.4] — 2026-09-16

### Added

- **shared:** El Agent Standard le dice al agente que lo leen en un MOVIL (#763)

## [2.75.3] — 2026-09-16

### Added

- **cli:** The agent can deploy, without ever seeing the deploy key (#758)

### Fixed

- **shared:** Gemini solo acepta API key — Google retiró el nivel OAuth (#761)

## [2.75.2] — 2026-09-15

### Added

- **shared:** Declare the credits_changed user event

### CI

- **release:** Wait up to 7.5 min for npm to serve the published version (#754)

### Fixed

- **cli:** A managed-rail outage must never be blamed on Anthropic (#757)

## [2.75.1] — 2026-09-12

### Fixed

- **cli:** Managed sessions announce the managed agent, not the Claude Code runtime (#753)

## [2.75.0] — 2026-09-12

### Added

- **shared:** Managed provider ids in @codeam/shared; CLI runs them on the house rail (#751)

## [2.74.14] — 2026-09-11

### Fixed

- **cli:** Stop dropping short ACP text deltas that are a prefix of the reply (#748)
- **cli:** Resolve select_option by optionId, stop adding `from` as an index offset (#749)
...
- **cli:** Start_task naming the session's own agent is a prompt, not an @-mention (#723)

## [2.74.9] — 2026-09-07

### Fixed

- **cli:** Expo preview through our own cloudflared tunnel — stop depending on Expo's shared ngrok (#720)
...
- **cli:** Daemon lock no longer defers to a reused pid — warm-codespace resume left the session dead (#715)

## [2.74.6] — 2026-09-05

### Fixed

- **cli:** Expo preview reads the tunnel URL a non-TTY Expo actually emits (exp.direct) (#713)

## [2.74.5] — 2026-09-04

### Chore

- **cli:** Bump @agentclientprotocol/claude-agent-acp 0.59.0 → 0.74.0 (Claude Code 2.1.257) (#711)
...
- **cli:** Mcp-warm spawns its launcher through a shell on Windows (#706)
- **cli:** Expo preview — make sure @expo/ngrok exists before `expo start --tunnel` (#707)

## [2.74.2] — 2026-09-04

### Fixed

- **cli:** A "no dev server" detection is a verdict, not a malformed answer (#704)

## [2.74.1] — 2026-09-04

### Fixed

- **cli:** Npx integration servers start with npm audit off; bound mcp-warm; 25-min fleet-box step (#701)
...
- **cli:** Re-resolve the integrations manifest from the backend at session start (#697)

## [2.73.9] — 2026-09-03

### Tests

- **cli:** Move the MCP licence audit to apps/cli — it broke the shared publish (#695)

## [2.73.8] — 2026-09-03

### Fixed

- **shared:** ClickUp MCP server had gone paid — switch to clickup-mcp-pro (MIT, 162 tools) (#694)

## [2.73.7] — 2026-09-03

### Fixed

- **cli:** LoadSession closes the fresh session it abandons — one agent, not two (#692)
...
- **shared:** Postman's MCP server never installed — npm refuses it (#688)
...
- **cli:** Debounce MCP reprovisions so a run of changes costs ONE respawn (#684)
...
- **cli:** Fleet_migrate_box_image — re-point a sleeping box at :latest

### Documentation

- **shared:** Warn that Trello's "Secret" is not the token

## [2.72.0] — 2026-09-02

### Added

- **cli:** Env_parse — import a pasted .env instead of typing variables one by one

### Fixed

- **cli:** No exagerar el radio del ack rechazado
...
- **cli:** House-agent outage copy must rule out a user charge
- **cli:** Raise the house-notice reply guard to 400 chars (measured)
...
- **cli:** Update onboarding Discord invite to the never-expiring link (#594)
