...
import { LAUNCHER_ENV } from '../../src/integrations/launcher-env';
...
async function listTools(command: string, args: string[], env: Record<string, string>) {
  // The SAME launcher env the `codeam mcp-run` shim uses — a cold `npx` without
  // it sat 268 s in `npm audit` on 2026-09-03 and this gate timed out, twice.
  const child = spawn(command, args, {
    env: { ...process.env, ...LAUNCHER_ENV, ...env },
  ...
  const msgs = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: 'codeam-test', version: '0' },
      },
    },
  ...
  const tools = await new Promise<string[]>((resolve, reject) => {
    const t = setTimeout(
      () => reject(new Error(`no tools/list answer in 180s. stderr: ${err.slice(-400)}`)),
      180_000,
    );
...
describe.runIf(enabled)(
  'the pinned ClickUp MCP server serves the tools users need, unpaywalled',
  () => {
    it('tools/list exposes task/space/comment tools and mentions no licence', async () => {
      ...
      for (const re of MUST_HAVE) {
        expect(
          tools.some((t) => re.test(t)),
          `no tool matching ${re} in: ${tools.join(', ')}`,
        ).toBe(true);
      }
