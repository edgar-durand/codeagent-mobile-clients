...
- **1M-context usage-credits gate** (`v2.43.0`, reworked): claude Code v2.1.x always sends the `context-1m` beta even on credit-less accounts → 429 "Usage credits required for 1M context" every turn. `looksLike1mContextCreditsError` classifies it; the recovery is to **reconnect the Claude subscription** via the in-app OAuth (a `codeam://reauth` bubble + `reportCredentialInvalid`), because the credential simply lacks the entitlement — the old "Disable 1M context and continue" `select_prompt` did NOT fix it (2026-06-24) and was removed.
