...
- **1M-context usage-credits gate (on-demand recovery, `v2.43.0`).** claude Code v2.1.x ALWAYS sends the `anthropic-beta: …,context-1m-2025-08-07` header even when the account has `s1mAccessCache.hasAccess=false`; a credit-less account then 429s every turn with "Usage credits required for 1M context" (confirmed in the codespace's `~/.headroom/logs/proxy.log` — claude's inbound request carries the beta; Headroom forwards it unchanged, NOT its fault). Detected by `looksLike1mContextCreditsError`; instead of a dead bubble the runner offers a tappable `select_prompt` "Disable 1M context and continue" (`oneMContextRecovery.ts`). On tap it persists `SavedSession.disable1mContext`, re-spawns the ACP adapter with `CLAUDE_CODE_DISABLE_1M_CONTEXT=1` (`AcpClient.extraEnv`), and re-runs the failed prompt. **On-demand only — never disabled for all users, no provisioning change.** The disable/re-spawn/re-run wiring is a DI factory (`createOneMRecovery`) so it's unit-tested with fakes.
